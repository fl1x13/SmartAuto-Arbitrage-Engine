...
1. **Tailscale Funnel** (`*.ts.net`) — постоянный адрес из `.env` (`WEBAPP_URL`).
   Используется, если провайдер его не блокирует.
2. **localhost.run** (`*.lhr.life`) — бесплатный запасной туннель без аккаунта.
   Включается автоматически, когда Tailscale недоступен.

Watchdog каждые 90 секунд проверяет, что текущий адрес реально отдаёт твоё
приложение, и при блокировке поднимает следующий, перезапуская бота с новым
адресом. Текущий рабочий адрес всегда лежит в
`~/Pet_project_fable/.autoradar/current_url.txt`.

### ⚠️ Чтобы адрес был ПОСТОЯННЫМ (один раз, бесплатно)

Анонимный `localhost.run` при каждом переподключении даёт **новый случайный**
адрес — старые ссылки перестают работать. Чтобы адрес не менялся, зарегистрируй
SSH-ключ на localhost.run (бесплатный «forever free» план):

1. Открой <https://admin.localhost.run/> и войди (можно через GitHub/Google).
2. Добавь публичный ключ. Он уже создан в проекте — скопируй его содержимое:
   ```bash
   cat ~/.ssh/lhr_tunnel_ed25519.pub
   ```
3. Готово. Supervisor уже настроен использовать этот ключ
   (`scripts/autoradar.sh`, переменная `LHR_KEY`) — после регистрации адрес
   `*.lhr.life` станет постоянным сам, без изменений в коде. Перезапусти стек:
   `launchctl kickstart -k gui/$(id -u)/com.autoradar`.
